...
from src.TokenBasedReplayForPartialOrders.structures._run import Run, EventForRun
from src.TokenBasedReplayForPartialOrders.structures._single_place_token_result import SinglePlaceTokenResult
from src.TokenBasedReplayForPartialOrders.structures._workflow_net import PlaceWorkflowNet
...
class Node4FlowNetwork:
    def __init__(self, node_type: NodeType, event: EventForRun, label: int = -1):
        self.type = node_type
        self.event = event
        self.excess: int = 0
        self.label = label
...
def find_optimal_tokenflow_for_place(place: PlaceWorkflowNet, run: Run, heuristic: SinglePlaceTokenResult) \
    ...
    def build_maximal_flow_problem(consumed_token: int, produced_token=0) -> MaxFlowNetwork:
        ...
        list_events: list[EventForRun] = list(partial_order_run)
