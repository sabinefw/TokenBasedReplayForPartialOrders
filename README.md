...
Algorithm for calculating an exact fitness value using token-based replay on partially ordered event logs in certain (true concurrent) semantics and workflow nets, and a fast algorithm variant for calculating upper and lower fitness bounds. Submitted to Petri Nets'26.
