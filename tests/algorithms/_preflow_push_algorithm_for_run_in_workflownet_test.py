...
from src.TokenBasedReplayForPartialOrders.algorithms._find_total_order_for_partial_order import find_total_order_for_run
from src.TokenBasedReplayForPartialOrders.algorithms._preflow_push_algorithm_for_run_in_workflownet import find_optimal_tokenflow_for_place
from src.TokenBasedReplayForPartialOrders.structures._run import EventForRun, Run
from src.TokenBasedReplayForPartialOrders.structures._single_place_token_result import SinglePlaceTokenResult
from src.TokenBasedReplayForPartialOrders.structures._workflow_net import PlaceWorkflowNet, TransitionWorkflowNet
...
class PreflowPushAlgorithmTestcases(unittest.TestCase):
    ...
    def test_first_example(self):
        ...
        :return:
        """
        event_1: EventForRun = EventForRun("Event 1", self.transition_nothing)
        event_2: EventForRun = EventForRun("Event 2", self.transition_nothing)
        event_3: EventForRun = EventForRun("Event 3", self.transition_only_producing)
        event_4: EventForRun = EventForRun("Event 4", self.transition_only_consuming)
        event_5: EventForRun = EventForRun("Event 5", self.transition_nothing)
        event_6: EventForRun = EventForRun("Event 6", self.transition_only_consuming)
        event_7: EventForRun = EventForRun("Event 7", self.transition_only_consuming)
    ...
    def test_second_example(self):
        event_1: EventForRun = EventForRun("Event 1", self.transition_producing_and_consuming)
        event_2: EventForRun = EventForRun("Event 2", self.transition_nothing)
        event_3: EventForRun = EventForRun("Event 3", self.transition_only_producing)
        event_4: EventForRun = EventForRun("Event 4", self.transition_nothing)
        event_5: EventForRun = EventForRun("Event 5", self.transition_nothing)
        event_6: EventForRun = EventForRun("Event 6", self.transition_only_producing)
        event_7: EventForRun = EventForRun("Event 7", self.transition_only_producing)
        event_8: EventForRun = EventForRun("Event 8", self.transition_only_consuming)
        event_9: EventForRun = EventForRun("Event 9", self.transition_only_consuming)
        partial_order: DiGraph = DiGraph()
    ...
    def test_third_example(self):
        ...
        event_1: EventForRun = EventForRun("Event 1", self.transition_nothing)
        event_2: EventForRun = EventForRun("Event 2", self.transition_nothing)
        event_3: EventForRun = EventForRun("Event 3", self.transition_only_consuming)
        event_4: EventForRun = EventForRun("Event 4", self.transition_only_producing)
        event_5: EventForRun = EventForRun("Event 5", self.transition_nothing)
        event_6: EventForRun = EventForRun("Event 6", self.transition_nothing)
        event_7: EventForRun = EventForRun("Event 7", self.transition_only_producing)
        event_8: EventForRun = EventForRun("Event 8", self.transition_nothing)
        event_9: EventForRun = EventForRun("Event 9", self.transition_nothing)
        partial_order: DiGraph = DiGraph()
        partial_order.add_edges_from(
    ...
    def test_fourth_example(self):
        ...
        event_5: EventForRun = EventForRun("Event 5", self.transition_nothing)
        event_6: EventForRun = EventForRun("Event 6", self.transition_nothing)
        event_7: EventForRun = EventForRun("Event 7", self.transition_nothing)
        event_8: EventForRun = EventForRun("Event 8", self.transition_nothing)
        event_9: EventForRun = EventForRun("Event 9", self.transition_nothing)
        event_10: EventForRun = EventForRun("Event 10", self.transition_nothing)
        event_11: EventForRun = EventForRun("Event 11", self.transition_only_consuming)
        event_12: EventForRun = EventForRun("Event 12", self.transition_nothing)
        partial_order: DiGraph = DiGraph()
        partial_order.add_edges_from(
            [(event_1, event_3), (event_2, event_3), (event_3, event_4), (event_3, event_5),
             (event_4, event_6), (event_5, event_6), (event_6, event_7), (event_7, event_8), (event_8, event_9),
